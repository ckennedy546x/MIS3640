...
class Text(object):
    ...
    def create_moved_dict(self, move):
        ...
        for ltr in lower:
            if lower.index(ltr) + move < 26:
                dict1[ltr] = lower[lower.index(ltr) + move]
            else:
                dict1[ltr] = lower[lower.index(ltr)-26+move]
    
        for ltr in upper:
            if upper.index(ltr) + move < 26:
                dict1[ltr] = upper[upper.index(ltr) + move]
            else:
                dict1[ltr] = upper[upper.index(ltr)-26+move]
...
class CipherText(Text):
    ...
    def decrypt_text(self):
        ...
        '''
        best_move = () #shift value, message text
        highest_word_count = 0
        
        for s in range(26):
            word_count = 0
            decrypted_str = self.apply_move(s)
            str_list = decrypted_str.split(' ')
            
            for item in str_list:
                if is_a_valid_word(self.valid_words, item):
                    word_count += 1
                    
            if word_count > highest_word_count:
                highest_word_count = word_count
                best_move = (s, decrypted_str)
